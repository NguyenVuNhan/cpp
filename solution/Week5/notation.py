...
class Notation:
    ...
    def toGraph(self, rootId, f):
        selfid = Id()   # Create unique id

        if(rootId != -1):
            f.write("\tnode{} -- node{}\n".format(rootId, selfid.id))

        f.write("\tnode{} [ label = \"{}\" ]\n".format(str(selfid.id), self.data)) #root
        
        if(self.nodeLeft != None):
            self.nodeLeft.toGraph(selfid.id, f)
        if(self.nodeRight != None):
            self.nodeRight.toGraph(selfid.id, f)
...
class Add(Notation):
    ...
    def simplify(self):
        ...
        if ( type(self.nodeLeft) == Mul):
            if ( self.nodeLeft.nodeLeft.toString() == self.nodeRight.toString() ):
                return Mul(Add(self.nodeLeft.nodeRight, Number(data="1").simplify()), self.nodeRight).simplify()
            elif ( self.nodeLeft.nodeRight.toString() == self.nodeRight.toString() ):
                return Mul(Add(self.nodeLeft.nodeLeft, Number(data="1").simplify()), self.nodeRight).simplify()
        if ( type(self.nodeRight) == Mul):
            if ( self.nodeRight.nodeLeft.toString() == self.nodeLeft.toString() ):
                return Mul(Add(self.nodeRight.nodeRight, Number(data="1").simplify()), self.nodeLeft).simplify()
            elif ( self.nodeRight.nodeRight.toString() == self.nodeLeft.toString() ):
                return Mul(Add(self.nodeRight.nodeLeft, Number(data="1").simplify()), self.nodeLeft).simplify()
        # ((f(x) + a(x)) + (f(x) + b(x)))
        if ( type(self.nodeLeft) == type(self.nodeRight) == Add ):
            ...
            if ( len(node_ll.toString()) <= len(node_lr.toString()) ):
                ...
                return node_lr
        # ln(a) + ln(b)
        if ( type(self.nodeLeft) == type(self.nodeRight) == Ln ):
            return Ln(Mul(self.nodeLeft.nodeLeft, self.nodeRight.nodeLeft).simplify())
        return self
    ...
    def getDerivative(self):
        ...
        if ( u.toString() == 0 and v.toString() == 0):
            return Number(data='0')
        if ( u.toString() == 0 ):
            return v
        if ( v.toString() == 0 ):
...
class Mul(Notation):
    ...
    def simplify(self):
        ...
        if ( type(self.nodeLeft) == Mul):
            if ( self.nodeLeft.nodeLeft.toString() == self.nodeRight.toString() ):
                return Mul(self.nodeLeft.nodeRight, Pow(self.nodeRight, Number(data="2").simplify())).simplify()
            elif ( self.nodeLeft.nodeRight.toString() == self.nodeRight.toString() ):
                return Mul(self.nodeLeft.nodeLeft, Pow(self.nodeRight, Number(data="2").simplify())).simplify()
        if ( type(self.nodeRight) == Mul):
            if ( self.nodeRight.nodeLeft.toString() == self.nodeLeft.toString() ):
                return Mul(self.nodeRight.nodeRight, Pow(self.nodeLeft, Number(data="2").simplify())).simplify()
            elif ( self.nodeRight.nodeRight.toString() == self.nodeLeft.toString() ):
                return Mul(self.nodeRight.nodeLeft, Pow(self.nodeRight, Number(data="1").simplify())).simplify()
        # e^a*e^b
        if( type(self.nodeLeft) == type(self.nodeRight) == E ):
            return E(Add(self.nodeLeft.nodeLeft,self.nodeRight.nodeLeft).simplify())
        return self
...
class Sub(Notation):
    ...
    def simplify(self):
        ...
        # ln(a) - ln(b)
        if ( type(self.nodeLeft) == type(self.nodeRight) == Ln ):
            return Ln(Div(self.nodeLeft.nodeLeft, self.nodeRight.nodeLeft).simplify())
        return self
...
class Div(Notation):
    ...
    def simplify(self):
        ...
        # e^a*e^b
        if( type(self.nodeLeft) == type(self.nodeRight) == E ):
            return E(Sub(self.nodeLeft.nodeLeft,self.nodeRight.nodeLeft).simplify())
        return self
...
class Pow(Notation):
    ...
    def simplify(self):
        ...
        if ( type(self.nodeLeft) == Number and
                type(self.nodeRight) == Number ):
            return Number(data=str(float(self.nodeLeft.data.replace(',','.'))**float(self.nodeRight.data.replace(',','.'))).replace(".0",""))
        if ( ( type(self.nodeLeft) == N or type(self.nodeLeft) == R ) and
                ( type(self.nodeRight) == N or type(self.nodeRight) == R ) ):
            return Number(data=str(float(self.nodeLeft.nodeLeft.data.replace(',','.'))**float(self.nodeRight.nodeLeft.data.replace(',','.'))).replace(".0",""))
        ...
        if ( self.nodeRight.data == '0' or
            ...
            return Number(data='1')
        # (e^a)^b
        if( type(self.nodeLeft) == E ):
            return E(Mul(self.nodeLeft.nodeLeft, self.nodeRight).simplify())
        return self
    ...
    def getDerivative(self):
        ...
        if ( u.toString() == 0 and v.toString() == 0 ):
            return Number
        if ( u.toString() == 0 ):
            node = Mul(Mul(v,Ln(self.nodeLeft)),Pow(self.nodeLeft, v))
            return node
        if ( v.toString() == 0 ):
            node = Mul(self.nodeRight, Pow(self.nodeLeft, Sub(self.nodeRight,Number(data="1"))))
            return node
        node = Add(Mul(Mul(Pow(self.nodeLeft, self.nodeRight),v),Ln(self.nodeLeft)),Mul(Mul(Pow(self.nodeLeft,Sub(self.nodeRight, Number(data="1"))),self.nodeRight),u))
        return node
...
class C(Notation):
    ...
    def getDerivative(self):
        ...
        if ( u.toString() == 0 ):
            return Number(data='0')
        return Mul(u, Mul(Number(data="-1"), S(self.nodeLeft)))
...
class E(Notation):
    ...
    def simplify(self):
        ...
        if ( self.nodeLeft.toString() == '0' ):
            return Number(data="1")
        # e^ln(f(x))
        if ( type(self.nodeLeft) == Ln ):
            return self.nodeLeft.nodeLeft.simplify()
        return self
...
class Ln(Notation):
    ...
    def simplify(self):
        ...
        if( self.nodeLeft.toString() == "1" ):
            return Number(data="0")
        if( self.nodeLeft.toString() == "(e^1)" ):
            return Number(data="1")
        # if( type(self.nodeLeft) == Pow ):
            # return Pow(self.nodeLeft.nodeRight.simplify(), Ln(self.nodeLeft.nodeLeft).simplify()).simplify()
        return self
