...
class MainForm:
    ...
    def plot(self):
        ...
        if ( self.option == "linear" ):
            ...
            for i in range(len(t)):
                try:
                    y[i] = self.getMcLaurinSeries(int(self.a_input.get()), t[i], depth=int(self.depth_input.get()))
                except:
                    y[i] = numpy.NAN
    ...
    def GUI(self):
        ...
        Radiobutton(self.frameRightOption, 
                    ...
                    padx=2, pady=2, 
                    command=lambda: self.setPlotFunction( func=("0" if (self.parse == None) else self.parse.function) ),
                    variable=v, 
                    value=0).grid(row=0, column=1, padx=2, sticky=W)
        Radiobutton(self.frameRightOption, 
                    text="Function derivative", 
                    padx=2, pady=2, 
                    command=lambda: self.setPlotFunction( func=("0" if (self.parse == None) else self.parse.function_da) ),
                    variable=v, 
                    value=1).grid(row=1, column=1, padx=2, sticky=W)
        Radiobutton(self.frameRightOption, 
                    text="Function diferrence quotion", 
                    padx=2, pady=2, 
                    command=lambda: self.setPlotFunction( func=("0" if (self.parse == None) else self.parse.function_dq) ),
                    variable=v, 
